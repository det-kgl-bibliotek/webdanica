...
if [ ! -f "$WEBDANICA_JAR" ]; then
   echo "ERROR: The WEBDANICA_JAR '$WEBDANICA_JAR' does not exist. Maybe the lib folder is missing, or the webdanica VERSION is wrong. Exiting program $PROG"
   exit 1
fi
...
CLASSPATH="$WEBDANICA_JAR:$PHOENIX_CLIENT_JAR:lib/*"